...
### 2. implement the different actions in Script:main() directly or with helper functions do_markdown
### 3. implement helper functions you defined in previous step
### ==============================================================================

### Created by pforret ( pforret ) on 2025-02-04
...
readonly script_created="2025-02-04"
readonly run_as_root=-1 # run_as_root: 0 = don't check anything / 1 = script MUST run as root / -1 = script MAY NOT run as root
readonly script_description="format a curlbash script in markdown"
...
option|D|DESCRIPTION|what does this script do|
option|R|REFERENCE|what URL explains when to use this|
choice|1|action|action to perform|markdown,action2,check,env,update
param|?|url|input url
" -v -e '^#' -e '^\s*$'
}
...
function Script:main() {
  ...
  case "${action,,}" in
  markdown)
    #TIP: use «$script_prefix markdown» to ...
    #TIP:> $script_prefix markdown
    [[ -z "$url" ]] && IO:die "need a URL to process"
    [[ ! "$url" =~ ^http ]] && IO:die "URL should start with http"
    domain=$(awk -F/ '{print $3}' <<<"$url")
    IO:debug "domain: $domain"
    pretty=$(cut -d/ -f3- <<<"$url" | sed 's/raw.githubusercontent.com//')
    digest=$(Str:digest 2 <<<"$url")
    IO:debug "digest: $digest"
    tempfile="$TMP_DIR/$domain.$digest.sh"
    curl -s "$url" > "$tempfile"
    output=$domain.$digest.md
    {
      echo "# $pretty"
      echo " "
      echo "## Description"
      echo "* run \`curl -s $url | bash\` to $DESCRIPTION"
      echo "* cf [$REFERENCE]($REFERENCE) "
      echo " "
      echo " "
      echo "## Stats"
      echo " "
      echo '```'
      cloc "$tempfile" | awk 'NR > 5 { print }'
      echo '```'
      echo "## Code"
      echo " "
      echo '```bash'
      cat $tempfile | fold -sw 110
      echo '```'
